...
import com.git.domain.ResponseMenu;
...
public class MessageController {
	...
	@Resource(name = "menuServiceKuaiDi")
	private MenuService menuServiceKuaiDi;
	...
	public ResponseEntity<String> getMsg(HttpServletRequest request) {
		try {
			...
			logger.info("用户" + xmlEntity.getFromUserName() + "在" + DateFormatUtils.format(new Date(), "HH:mm:ss")
					+ "发来消息:" + xmlEntity.getContent());
	...
	public ResponseEntity<String> getMsg2(HttpServletRequest request) {
		try {
			...
			XmlEntity xmlRes = this.menuServiceKuaiDi.exec(xmlEntity.getContent(), xmlEntity);
